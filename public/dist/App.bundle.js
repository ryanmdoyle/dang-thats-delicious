  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      ...
      var marker = new google.maps.Marker({
        map: map,
        position: position //like map:map, position: position
      });
...
} // an ajax package that has some addl. functionality
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    //.on is from bling.js, same as eventlistener
    ...
    if (this.value.length > 2) {
      //reduces api calls to searches with 3+ characters
      _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
        ...
        //don't forget you have to catch promises!
        console.error(err);
      });
    }
  });
